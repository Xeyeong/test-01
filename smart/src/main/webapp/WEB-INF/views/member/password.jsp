		...
		<form action="change" method="post">
			<ul>
				<li><input type="password" name="pw" placeholder="새 비밀번호" />
					<div class="valid text-left">비밀번호를 입력하세요(영문대소문자,숫자 포함)</div></li>
				<li><input type="password" name="pw_ck" placeholder="새 비밀번호 확인" />
					<div class="valid text-left">비밀번호를 다시 입력하세요</div></li>
				<li><input type="reset" value="다시입력" /></li>
				<li><input type="button" value="비밀번호변경" /></li>
			</ul>
		</form>
</div>
...
function tagIsValid(){
	...
	$('[type=password]').keyup(function(){
		var status = member.tag_status($(this));
		$(this).siblings('div').removeClass('valid invalid').addClass(status.code).text(status.desc);
	});
</script>
</body>
</html>